...
app.get("/alldevs", function (req, res) {
    developer.find().exec(function (err, data) {
        if (err) {
            ...
            console.log(data);
            res.render(pathName+'alldevelopers.html', { devDb: data });
            //res.send(data);
        }
    });
});
...
app.post('/taskUpdate', function (req, res) {  // updates tasks
    task.updateOne({ '_id': req.body.tId }, { $set: { 'status': req.body.tStatus } }).exec( function (err) {
        if (err) {
            throw err;
        } else {
            console.log(req.body);
...
app.post('/deleteTask', function (req, res) {
    task.deleteOne({ '_id': req.body.tId }).exec(function (err) {
        if (err) {
            throw err;
        } else {
            console.log(req.body);
...
app.get('/deleteall', function (req, res) {
    task.deleteMany({ 'status': 'Complete' }).exec(function (err, doc) {
        if (err) {
            throw err;
        } else {
            console.log(req.body);
...
app.get('/sorttask', function (req, res) {
    let sortBy={tName:-1};
    task.find({'status':'Complete'}).sort(sortBy).limit(3).exec(function(err, doc){
        if(err) throw err;
        else{
            res.send(doc)
        }
    })
    //res.sendFile(pathName + "sorttask.html");
});
